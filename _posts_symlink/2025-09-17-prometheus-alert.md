...
在配置完 prometheus 和 grafana 一套监控系统之后，对于服务器或者 pod 的异常捕获和告警通知也是十分重要的。

![在这里插入图片描述](https://raw.githubusercontent.com/Holmses/Holmses.github.io/master/assets/img/9e42963782c340e19da3aceb84e4e62d.png)

本文采用 grafana 把告警推给 Prometheus-alert 由  Prometheus-alert 分发给飞书和钉钉。

## Prometheus-alert 部署处理告警信息
...
钉钉获取指南 [PrometheusAlert/doc/readme/conf-dingding.md at master · feiyu563/PrometheusAlert](https://github.com/feiyu563/PrometheusAlert/blob/master/doc/readme/conf-dingding.md)
...
> 注意名称全英文，模板用途 Prometheus 显示出来的路径是用来填写到 Grafana 联络点中的 Url
...
*[{{$v.labels.alertname}}]({{$v.annotations.alertURL}})*
<font color="green">[告警状态]</font>：{{printf "%s已恢复" $v.status}}
<font color="green">[开始时间]</font>：{{$v.startsAt}}
<font color="green">[结束时间]</font>：{{$v.endsAt}} 
<font color="green">[故障主机]</font>：{{$v.labels.instance}}
**<font color="green">{{$v.annotations.description}}{{printf "%.2f" $v.values.B}}%</font>**{{else}}**[查看告警信息仪表盘]({{$v.annotations.panelURL}})**❌
*[{{$v.labels.alertname}}]({{$v.annotations.alertURL}})*
<font color="red">[告警状态]</font>：{{printf "%s告警中" $v.status}}
<font color="red">[开始时间]</font>：{{$v.startsAt}}
<font color="red">[故障主机]</font>：{{$v.labels.instance}}
**<font color="red">{{$v.annotations.description}}{{printf "%.2f" $v.values.B}}%</font>**{{end}}{{ end }}
...
1. [Kubernetes中部署PrometheusAlert并使用mysql作后端存储_51CTO博客_kubernetes mysql](https://blog.51cto.com/wutengfei/4256244)
2. [PrometheusAlert/doc/readme/system-customtpl.md at master · feiyu563/PrometheusAlert](https://github.com/feiyu563/PrometheusAlert/blob/master/doc/readme/system-customtpl.md)
